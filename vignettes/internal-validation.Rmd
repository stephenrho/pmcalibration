...
2. Resample with replacement from the development data to create a boostrap sample with same number of observations. 
...
5. Optimism is the difference in performance between the model as evaluated on the bootsrap sample and the original sample: $M_{opt} = M_{boot} - M_{orig}$
...
internal_cal <- function(model_fun, data, pred_fun, y="y", B=100){
  ...
  one_boot <- function(model, data, pred_fun){
    ...
    p_boot <- pred_fun(bootmodel, d) # evaluate on 'training' data (boostrap resample)
  ...
  metrics_opt <- rbindlist(lapply(opt, \(x) data.frame(t(x[[1]]))))
  metrics_opt <- apply(metrics_opt, 2, mean)
  plot_opt <- rbindlist(lapply(opt, \(x) data.frame(t(x[[2]]))))
